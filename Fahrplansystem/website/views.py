...
# Zeigt in der Registerkarte Home den Fahrplan an
@views.route("/")
@views.route("/home")
@login_required
def home():
...
# Erstellt neue Fahrtdurchführungen
...
def create_post():
    ...
    if request.method == "POST":
        ...
        # Checken ob Streckenwahl zulässig ist
        ...
        # Kontrollieren ob nur eine Checkbox von Datum und Wochentag gewählt wurde sowie nur eine Checkbox von Uhrzeit und Intervall
        if checkbox_tag:
        ...
        # Kontrollieren ob der Fahrtpreis die Kosten abdeckt
        ...
        # Kontrollieren ob die Spurweite des Zugs mit der Spurweite der Strecke übereinstimmt
        if methods.get_spurweite(str(zug),str(strecken)):
            flash('Spurweite passt nicht, wähle einen anderen Zug!', category='error')
            return render_template('create_post.html', user=current_user, users=users, plannedRoutes=plannedRoutes, zuege=zuege)

        # Wenn Datum ausgewählt wurde, werden alle Daten durchlaufen und zu jeder Uhrzeit/Intervall wird eien Fahrdurchführung erstellt.
        if checkbox_tag:
            date_list = datum.split(",")
            for d in date_list:
                # Wenn Uhrzeit gewählt wurde:
                if checkbox_uhrzeit:
                    ...
                    for t in time_list:
                        ...
                        # Kontrolle ob Zug und Bordpersonal verfügbar sind
                ...
                # Wenn Intervall gewählt wurde:
                    ...
                    while datetime_von <= datetime_bis:
                        ...
                        if methods.get_verfügabar_bordpersonal(bordpersonal,str(datetime_von.time())[0:5],str(datetime_ankunft.time())[0:5],d):
                            flash('Ausgewähltes Bordpersonal ist zum ausgewählten Zeitpunkt ' + str(datetime_von.time())[0:5] + ' Uhr am ' + d + ' nicht verfügbar!', category='error') 
                            test=False
                        if test:
        ...
        # Wenn Wochentag ausgewählt wurde, werden diese durchlaufen und dieFahrdurchführungen mit Uhrzeit/Intervall erstellt.
        else:
            date_list = wochentag.split(",")
            final_date_list = []
            # Liste von Wochentagen wird durchlaufen
            for d in date_list:
                i = 1
                anzahl_wochen = int(request.form.get('anzahl_wochen')) + 1

                # Jeder Wochentage wird n mal aufgerufen
                while i < anzahl_wochen:
                    ...
                    if checkbox_uhrzeit:
                        ...
                        for t in time_list:
                            ...
                            # Kontrolle ob Zug & Bordpersonal verfügbar sind.
                            ...
                            # Erstellen der Fahrtdurchführung
                            if test:
...
# Löscht Fahrtdurchführung mit gewünschter Id
...
# Gibt alle Fahrtdurchführungen eines Benutzers zurück
...
# Bordpersonal,Zug und Preis einer Fahrdurchführung ändern
...
# Zug einer Fahrdurchführung ändern
@views.route("/bearbeiten/zug/<id>", methods=['GET', 'POST'])
@login_required
def bearbeiten_zug(id):
    zuege = methods.get_zuege()
    users = User.query.all()
    bordpersonal_bisher = ''
    preis_bisher = ''
    ...
    return render_template('bearbeiten_zug.html', user=current_user, id=id, users=users, bordpersonal_bisher=bordpersonal_bisher, preis_bisher=preis_bisher, min_preis=min_preis,zug_bisher=zug_bisher, zuege=zuege)

# Preis einer Fahrdurchführung ändern
@views.route("/bearbeiten/preis/<id>", methods=['GET', 'POST'])
@login_required
def bearbeiten_preis(id):
    zuege = methods.get_zuege()
    users = User.query.all()
    bordpersonal_bisher = ''
    preis_bisher = ''
    ...
    return render_template('bearbeiten_preis.html', user=current_user, id=id, users=users, bordpersonal_bisher=bordpersonal_bisher, preis_bisher=preis_bisher, min_preis=min_preis,zug_bisher=zug_bisher, zuege=zuege)

# Bordpersonal einer Fahrdurchführung ändern
@views.route("/bearbeiten/bordpersonal/<id>", methods=['GET', 'POST'])
@login_required
def bearbeiten_bordpersonal(id):
    zuege = methods.get_zuege()
    users = User.query.all()
    bordpersonal_bisher = ''
    preis_bisher = ''
    ...
    return render_template('bearbeiten_bordpersonal.html', user=current_user, id=id, users=users, bordpersonal_bisher=bordpersonal_bisher, preis_bisher=preis_bisher, min_preis=min_preis,zug_bisher=zug_bisher, zuege=zuege)
