                ...
                if checkbox_uhrzeit:
                    ...
                    for t in time_list:
                        ...
                        if api.get_besetzt(zug,t,str(datetime_ankunft.time())[0:5],d):
                            ...
                            return render_template('create_post.html', user=current_user, users=users, plannedRoutes=plannedRoutes, zuege=zuege)
                        if api.get_wartung(zug,t,str(datetime_ankunft.time())[0:5],d):
                            flash('Der Ausgewählter Zug befindet sich zum ausgewählten Zeitpunkt ' + t + ' Uhr am ' + d + ' in der Wartung!', category='error')  
                            return render_template('create_post.html', user=current_user, users=users, plannedRoutes=plannedRoutes, zuege=zuege)
                        if api.get_verfügabar_bordpersonal(bordpersonal,t,str(datetime_ankunft.time())[0:5],d):
                            flash('Ausgewähltes Bordpersonal ist zum ausgewählten Zeitpunkt ' + t + ' Uhr am ' + d + ' nicht verfügbar!', category='error')  
                            return render_template('create_post.html', user=current_user, users=users, plannedRoutes=plannedRoutes, zuege=zuege)
                        else:
                            post = Post(ankunft=str(datetime_ankunft.time())[0:5],start=str(api.get_station((api.get_start(strecken)))), ziel=str(api.get_station(api.get_end(strecken))), zug=zug, bordpersonal=bordpersonal, preis=preis, datum=d, uhrzeit=t, strecken=strecken, author=current_user.id)
                            db.session.add(post)
                            db.session.commit()
                    ...
                    while datetime_von <= datetime_bis:
                        ...
                        if api.get_besetzt(zug,str(datetime_von.time())[0:5],str(datetime_ankunft.time())[0:5],d):
                            ...
                            return render_template('create_post.html', user=current_user, users=users, plannedRoutes=plannedRoutes, zuege=zuege)
                        if api.get_wartung(zug,str(datetime_von.time())[0:5],str(datetime_ankunft.time())[0:5],d):
                            ...
                            return render_template('create_post.html', user=current_user, users=users, plannedRoutes=plannedRoutes, zuege=zuege)
                        if api.get_verfügabar_bordpersonal(bordpersonal,str(datetime_von.time())[0:5],str(datetime_ankunft.time())[0:5],d):
                            flash('Ausgewähltes Bordpersonal ist zum ausgewählten Zeitpunkt ' + t + ' Uhr am ' + d + ' nicht verfügbar!', category='error')  
                            return render_template('create_post.html', user=current_user, users=users, plannedRoutes=plannedRoutes, zuege=zuege)
                        else:
                            post = Post(ankunft=str(datetime_ankunft.time())[0:5], start=str(api.get_station((api.get_start(strecken)))), ziel=str(api.get_station(api.get_end(strecken))), zug=zug, bordpersonal=bordpersonal, preis=preis, datum=d, uhrzeit=str(datetime_von.time())[0:5], strecken=strecken, author=current_user.id)
                            db.session.add(post)
                            db.session.commit()
                    ...
                    if checkbox_uhrzeit:
                        ...
                        for t in time_list:
                            ...
                            if api.get_besetzt(zug,t,str(datetime_ankunft.time())[0:5],str(nextDay.strftime("%d.%m.%y"))):
                                ...
                                return render_template('create_post.html', user=current_user, users=users, plannedRoutes=plannedRoutes, zuege=zuege)
                            if api.get_wartung(zug,t,str(datetime_ankunft.time())[0:5],str(nextDay.strftime("%d.%m.%y"))):
                                ...
                                return render_template('create_post.html', user=current_user, users=users, plannedRoutes=plannedRoutes, zuege=zuege)
                            if api.get_verfügabar_bordpersonal(bordpersonal,t,str(datetime_ankunft.time())[0:5],str(nextDay.strftime("%d.%m.%y"))):
                                flash('Ausgewähltes Bordpersonal ist zum ausgewählten Zeitpunkt ' + t + ' Uhr am ' + d + ' nicht verfügbar!', category='error')  
                                return render_template('create_post.html', user=current_user, users=users, plannedRoutes=plannedRoutes, zuege=zuege)
                            else:
                                post = Post(ankunft=str(datetime_ankunft.time())[0:5],start=str(api.get_station((api.get_start(strecken)))), ziel=str(api.get_station(api.get_end(strecken))), zug=zug, bordpersonal=bordpersonal, preis=preis, datum=str(nextDay.strftime("%d.%m.%y")), uhrzeit=t, strecken=strecken, author=current_user.id)
                                db.session.add(post)
                                db.session.commit()
                        ...
                        while datetime_von <= datetime_bis:
                            ...
                            if api.get_besetzt(zug,str(datetime_von.time())[0:5],str(datetime_ankunft.time())[0:5],str(nextDay.strftime("%d.%m.%y"))):
                                ...
                                return render_template('create_post.html', user=current_user, users=users, plannedRoutes=plannedRoutes, zuege=zuege)
                            if api.get_wartung(zug,str(datetime_von.time())[0:5],str(datetime_ankunft.time())[0:5],str(nextDay.strftime("%d.%m.%y"))):
                                ...
                                return render_template('create_post.html', user=current_user, users=users, plannedRoutes=plannedRoutes, zuege=zuege)
                            if api.get_verfügabar_bordpersonal(bordpersonal,t,str(datetime_von.time())[0:5],str(datetime_ankunft.time())[0:5],str(nextDay.strftime("%d.%m.%y"))):
                                flash('Ausgewähltes Bordpersonal ist zum ausgewählten Zeitpunkt ' + t + ' Uhr am ' + d + ' nicht verfügbar!', category='error')  
                                return render_template('create_post.html', user=current_user, users=users, plannedRoutes=plannedRoutes, zuege=zuege)
                            else:
                                post = Post(ankunft=str(datetime_ankunft.time())[0:5], start=str(api.get_station((api.get_start(strecken)))), ziel=str(api.get_station(api.get_end(strecken))), zug=zug, bordpersonal=bordpersonal, preis=preis, datum=str(nextDay.strftime("%d.%m.%y")), uhrzeit=str(datetime_von.time())[0:5], strecken=strecken, author=current_user.id)
                                db.session.add(post)
                                db.session.commit()
...
def getfahrplan_per_id(id):
    ...
    fahrten = api.get_fahrten_von_bordpersonal(user.id)
    fahrten_liste = []
...
def bearbeiten(id):
    ...
    for d in data:
        if str(d.id) == str(id):
            ...
            min_preis=str(api.get_preis_von_fahrtstrecke(d.strecken))
        ...
        if not post:
            ...
            if request.method == "POST":
                ...
                for d in data:
                    if str(d.id) == str(id):
                        if bordpersonal != '':
                            if api.get_verfügabar_bordpersonal(bordpersonal,d.uhrzeit,d.ankunft,d.datum):
                        ...
                        if zug != '' and zug != d.zug:
                            ...
                            if api.get_wartung(zug,d.uhrzeit,d.ankunft,d.datum):
                                ...
                                return render_template('bearbeiten.html', user=current_user, id=id, users=users, bordpersonal_bisher=bordpersonal_bisher, preis_bisher=preis_bisher, min_preis=min_preis,zug_bisher=zug_bisher, zuege=zuege)
                            if api.get_spurweite(str(zug),str(d.strecken)):
                                flash('Spurweite passt nicht, wähle einen anderen Zug!', category='error')
                                return render_template('bearbeiten.html', user=current_user, id=id, users=users, bordpersonal_bisher=bordpersonal_bisher, preis_bisher=preis_bisher, min_preis=min_preis,zug_bisher=zug_bisher, zuege=zuege)
