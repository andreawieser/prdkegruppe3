                    ...
                    if checkbox_uhrzeit:
                        ...
                        for t in time_list:
                            ...
                            if api.get_besetzt(zug,t,str(datetime_ankunft.time())[0:5],d):
                                ...
                                post = Post(Ankunft=str(datetime_ankunft.time())[0:5],startBhf=str(api.get_station((api.get_start(strecken)))), endBhf=str(api.get_station(api.get_end(strecken))), Zug=zug, bordpersonal=bordpersonal, Preis=preis, Datum=d, Abfahrt=t, author=current_user.id)
                                db.session.add(post)
                                db.session.commit()
                        ...
                        while datetime_von <= datetime_bis:
                            ...
                            if api.get_besetzt(zug,str(datetime_von.time())[0:5],str(datetime_ankunft.time())[0:5],d):
                                ...
                                post = Post(Ankunft=str(datetime_ankunft.time())[0:5], startBhf=str(api.get_station((api.get_start(strecken)))), endBhf=str(api.get_station(api.get_end(strecken))), Zug=zug, bordpersonal=bordpersonal, Preis=preis, Datum=d, Abfahrt=str(datetime_von.time())[0:5], author=current_user.id)
                                db.session.add(post)
                                db.session.commit()
                        ...
                        if checkbox_uhrzeit:
                            ...
                            for t in time_list:
                                ...
                                if api.get_besetzt(zug,t,str(datetime_ankunft.time())[0:5],str(nextDay.strftime("%d.%m.%y"))):
                                    ...
                                    post = Post(Ankunft=str(datetime_ankunft.time())[0:5],startBhf=str(api.get_station((api.get_start(strecken)))), endBhf=str(api.get_station(api.get_end(strecken))), Zug=zug, bordpersonal=bordpersonal, Preis=preis, Datum=str(nextDay.strftime("%d.%m.%y")), Abfahrt=t, author=current_user.id)
                                    db.session.add(post)
                                    db.session.commit()
                            ...
                            while datetime_von <= datetime_bis:
                                ...
                                if api.get_besetzt(zug,str(datetime_von.time())[0:5],str(datetime_ankunft.time())[0:5],str(nextDay.strftime("%d.%m.%y"))):
                                    ...
                                    post = Post(Ankunft=str(datetime_ankunft.time())[0:5], startBhf=str(api.get_station((api.get_start(strecken)))), endBhf=str(api.get_station(api.get_end(strecken))), Zug=zug, bordpersonal=bordpersonal, Preis=preis, Datum=str(nextDay.strftime("%d.%m.%y")), Abfahrt=str(datetime_von.time())[0:5], author=current_user.id)
                                    db.session.add(post)
                                    db.session.commit()
